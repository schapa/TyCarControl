                                        ...
                                        uint32_t lr)
{
  ...
  trace_printf ("\tR0 =  %08X", frame->r0);
  trace_printf ("\tR1 =  %08X", frame->r1);
  trace_printf ("\tR2 =  %08X", frame->r2);
  trace_printf ("\tR3 =  %08X", frame->r3);
  trace_printf ("\tR12 = %08X", frame->r12);
  trace_printf ("\tLR =  %08X", frame->lr);
  trace_printf ("\tPC =  %08X", frame->pc);
  trace_printf ("\tPSR = %08X", frame->psr);
  trace_printf ("FSR/FAR:");
  trace_printf ("\tCFSR =  %08X", cfsr);
  trace_printf ("\tHFSR =  %08X", SCB->HFSR);
  trace_printf ("\tDFSR =  %08X", SCB->DFSR);
  trace_printf ("\tAFSR =  %08X", SCB->AFSR);

  if (cfsr & (1UL << 7))
    {
      trace_printf ("\tMMFAR = %08X", mmfar);
    }
  if (cfsr & (1UL << 15))
    {
      trace_printf ("\tBFAR =  %08X", bfar);
    }
  trace_printf ("Misc");
  trace_printf ("\tLR/EXC_RETURN= %08X", lr);
}
